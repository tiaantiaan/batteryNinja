 ...
 * D4   Temperature sensor data pin (Texas Instruments ds18b20)
 * D1   OLED I2C display SCL
 * D2   OLED I2C display SCA
 *
 *******************************
 * Programming
 *
 * The MQTT web updater included in this project supports updating the firmmware via OTA.
 * Just go to the devices IP address in the browser and upload the build binary.
 */
...
EspMQTTClient mqttClient(
  "MYWIFIAP",           // Wifi Accesspoint
  "PASSWORD",           // Wifi password
  "192.168.1.200",      // MQTT Broker server ip
  "MQTTUsername",       // Can be omitted if not needed
  "",                   // Can be omitted if not needed
  "batteryninja"        // Client name that uniquely identifies your device
);
...
#define ONE_WIRE_BUS D5 // GPIO 14 - One wire pin for Temperature sensors
OneWire oneWire(ONE_WIRE_BUS);
DallasTemperature sensors(&oneWire);
float temperature1 = 0;
float previousTemperature1 = 0;
float temperature2 = 0;
float previousTemperature2 = 0;
int deviceCount = 0; // Used for multiple temp sensors on the same bus
...
noDelay connectionCheckInterval(1000);

// LED setup - Displays WIFI and MQTT connected status
#define LED_WIFI_PIN D6  // GPIO 12
#define LED_MQTT_PIN D7  // GPIO 13
...
void setup()
{
  ...
  mqttClient.enableHTTPWebUpdater("/"); // Enable the web updater. This will host a simple form that will allow firmware upgrade (using, e.g., the .bin file produced by "Export Compiled Binary" in the Arduino IDE's "Sketch" menu).
}
...
void loop()
{
  mqttClient.loop(); // Required for MQTT library to work

  // Check wifi and MQTT connection status
  if (connectionCheckInterval.update()) {
    // Display connection status
    digitalWrite(LED_MQTT_PIN, mqttClient.isMqttConnected()); 
    digitalWrite(LED_WIFI_PIN, mqttClient.isWifiConnected());
  }
 ...
 if (flickerInterval.update()) {
  ...
  if (firstrun || updateInterval.update()) {
    ...
    batteryVoltage = (sensorValue * 32)/1024; // Given a 62000 to 2000 voltage divider
    ...
    if (previousTemperature1 != temperature1) {
      previousTemperature1 = temperature1;
      mqttClient.publish("tele/batteryninja/TEMPERATURE1", "{\"temperature\":" + String(temperature1) + "}");
    }
    if (previousTemperature2 != temperature2) {
      previousTemperature2 = temperature2;
      mqttClient.publish("tele/batteryninja/TEMPERATURE2", "{\"temperature\":" + String(temperature2) + "}");
    }
   ...
   updateDisplay(temperature1, batteryVoltage, batteryPercentage);
  }
}
